...
# 获取当前脚本所在的目录
sys.path.append("/Users/changgeng.zhang/workspace/pay_trial/Robot_Data_Integration_Synchronization_Project")
...
def convert_reporting_machine_tool(device_mapping_dict,
    ...
    if machine_tools is None or not machine_tools:
        return scheduling_machine_tool, True, process_type_mapping_dict[scheduling_machine_tool]
    ...
    process_type_different = reporting_machine_tool_process_type == scheduling_machine_tool_process_type
    return reporting_machine_tool, process_type_different, reporting_machine_tool_process_type
...
def retrieve_production_reports():
    ...
    """
    # 请求服务端数据
    res = requests_post(config_manager.get_retrieve_production_reports_url())
    if res.status_code == 200:
        ...
        MessageSender(MessageType.DINGTALK,
                      BusinessType.RETRIEVE_PRODUCTION_REPORTS.name,
                      MessageLevel.ERROR).send_message("服务请求异常, Failed status code: {}".format(res.status_code))


def test_retrieve_production_reports():
    return utils.read_excel_tuple_list('/Users/changgeng.zhang/data/生产报工/2024-01-11/1704941586.xlsx', 1)
